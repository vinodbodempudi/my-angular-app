...
app.directive('popOver', ['$compile', 'FatHomeAppStateUtil', '$rootScope', 'PropertiesService', 'FatHomeUtil', function ($compile, fatHomeAppStateUtil, $rootScope, propertiesService, fatHomeUtil) {
  ...
  return {
    ...
    link: function (scope, element, attrs) {
		...
		scope.soldOutProperty = function($event, propertyId) {
			$event.stopPropagation();
			if (window.confirm('Are you sure you want sold out?')) {
				var request = {_id:propertyId, update:{active:'S'}}
				propertiesService.updateProperty(request)
				.success(function(data){
					alert('Property marked as sold out.');
				}).error(function(e){
